...
clf = SVC(kernel='linear', probability=True,
          tol=1e-3)  # , verbose = True) #Establece el clasificador como un vector de apoyo de máquinas polinomial kernel
...
def get_landmarks(image):
    ...
    for k, d in enumerate(detections):  # Para todas las cara detectadas de forma individual
        ...
        if xlist[26] == xlist[
            29]:  # Si la coordenada x del conjunto son las mismas, el ángulo es 0,  evitamos el error 'divide by 0' en la función
            anglenose = 0
        else:
